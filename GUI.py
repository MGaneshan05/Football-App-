...
def format_response2(info):
    try:
      team_name = info['response']["team"]['name']
      form = info['response']["form"]
      form1 = str(form[20:28])
      played = info['response']["fixtures"]['played']['total']
      won = info['response']["fixtures"]['wins']['total']
      draw = info['response']["fixtures"]['draws']['total']
      loss = info['response']["fixtures"]['loses']['total']
      scored = info['response']["goals"]['for']['total']['total']
      conceded = info['response']["goals"]['against']['total']['total']
      winstrk = info['response']["biggest"]['streak']['wins']
      clnsht = info['response']["clean_sheet"]["total"]
      basic_info = 'Team Name: %s \nCurrent Form: %s \nGames Played" %s \nWins: %s \nDraws: %s \nLosses: %s \nScored: ' \
                   '%s \nConceded: %s \nLongest Winstreak: %s \nClean Sheets: %s ' %(team_name,form1,played,won,draw,loss,
                                                                                     scored,conceded,winstrk,clnsht)
...
def tm1(entry1, entry2, entry3, entry4):
   try:
       ...
       label1['text'] = format_response2(info)
   ...
   return info
def tm2(entry1, entry2, entry3, entry4):
   try:
       if switch == False:
           x = "#26242f"
       ...
       label2['text'] = format_response2(info)
       ...
       button3 = tk.Button(upper_frame, text='Clear Slot 2', font=('Trebuchet MS', 15), command=lambda: clear())


       button3.place(relx=0.85, rely=0.6, relwidth=0.15, relheight=0.4)
   except:
       print('sad')
       label1.config(text='There was an error processing \nyour request.'
                          '\nPlease check your spelling \nand try again.')
...
button7 = tk.Button(upper_frame, bd=0, text='Team View Slot 2', command=lambda: tm2(entry1.get(),
                                                                                   entry2.get(), entry3.get(),
                                                                                   entry4.get(),))
